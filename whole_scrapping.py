...
def scrapping_all():
    ...
    with webdriver.Chrome("C:/Users/Alonso Uribe/AppData/Local/Chromium/User Data/chromedriver.exe") as driver:
        ...
        menu_class = "search-bar "
        menu_xpath = f"//div[@class='{menu_class}']"
        menu_pointer = driver.find_element(By.XPATH, menu_xpath)
        pointers_class = "andes-dropdown andes-dropdown--standalone  searchbox-dropdown "
        pointers_xpath = f"//div[@class='{pointers_class}']"
        ...
        for property_type in properties_pointer.find_elements(By.TAG_NAME, 'li'):
            properties.append(property_type.text.lower())
        webdriver.ActionChains(driver).move_to_element(properties_pointer).send_keys(Keys.ESCAPE).perform()
        ...
        for sell in sells:
            ...
            for property in properties:
                webdriver.ActionChains(driver).move_to_element(properties_pointer).click.perform()
                ...
                #nxt_url = f"{curr_url}{sell}/{property}/"
                max_iter = 100
